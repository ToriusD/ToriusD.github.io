...
new TextBlock('Государственное учреждение культуры «Централизованная библиотечная система г.Новополоцка» (далее ЦБС) включает семь библиотек, из них две детские и три семейного типа.', {
  ...
  new ColumnBlock([
    alink('#', 'Год мира и созидания', text_secondary),
    alink('#', 'Время.События.Люди', text_secondary),
    alink('#', 'Центр правовой информации', text_secondary),
    alink('#', 'Новополоцк молодежная столица 2023', text_secondary)
  ], {
      styles: {
...
function navbar(content, image, style='') {
  return `
    <nav class="navbar sticky-top navbar-expand-lg navbar-dark bg-primary" style="${style}">
